...
export default {
  ...
  methods: {
    searchMap() {
      ...
      axios
        .get(
          `https://dapi.kakao.com/v2/local/search/keyword.json?y=37.498721&x=127.0329693&radius=20000&query=${search}`,
          {
            headers: {
        ...
        .then((res) => {
          ...
          for (let i = 0; i < list.length; i++) {
            this.resultList.push({
              address: list[i].road_address_name,
              name: list[i].place_name,
              x: list[i].x,
              y: list[i].y,
              phone: list[i].phone,
              url: list[i].place_url,
            });
          }
