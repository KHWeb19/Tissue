<template>
  <div align="center">
    ...
    <v-container>
      ...
      <v-card style="width: 100%" class="detailSearchBox" flat>
        <v-row>
          <v-card-title class="detailSearchTxt">
            상세검색
            <v-icon v-if="!detailSearch" @click="detailSearch = true"
              >mdi-plus</v-icon
            >
          ...
          <v-card-title>
            <v-tabs color="blue lighten-3" background-color="transparent">
              <v-tabs-slider color="pink lighten-3"></v-tabs-slider>
              <v-tab class="sortbtn black--text" @click="sortRecent"
                >최신순</v-tab
              >
              <v-tab class="sortbtn black--text" @click="sortEnd"
                >종료임박순</v-tab
              >
            </v-tabs>
          </v-card-title>
        ...
        <v-card-actions v-if="detailSearch">
          <v-container class="pt-0" fluid>
            ...
            <v-btn @click="applyFilter" class="mr-3" color="blue lighten-3"
              >적용</v-btn
            >
      ...
      <div class="countBox">
        현재 예매가능한 공연은
        <b style="color: skyblue">{{ copyPerformList.length }}개</b>입니다.
      </div>

      <v-row>
        <v-col
          v-for="perform in this.copyPerformList"
          :key="perform.performNo"
          lg="3"
          sm="6"
        >
